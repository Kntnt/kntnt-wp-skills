...
Every decision is presented as a recommendation with an accept-or-override gate. When a saved plan exists for the site, the whole walk collapses to a single *replay the saved plan?* gate, which `--yes` runs silently.
