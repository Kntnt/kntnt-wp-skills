...
The skill reaches production solely through the Kntnt Extractor plugin's REST API, authenticated with an Application Password; there is no SSH. Before anything else it runs a health check that verifies every local and production dependency the run needs — `ddev`, the required CLI tools, and the target site's Kntnt Extractor endpoint (its API-version handshake and that the Application Password's user holds both required capabilities) — with guided remediation the moment anything is missing, then proves the channel is live, confirms it targets production, sweeps stranded jobs from an aborted earlier run, and preflights the download path. It then diffs production against the stored baseline manifest, so the extraction carries only new and changed files, which are downloaded and unsealed; the remote job is consumed immediately after the download unseals. The database is always dumped in full. Before importing, the skill verifies the local table prefix still matches production and aborts on a mismatch.
