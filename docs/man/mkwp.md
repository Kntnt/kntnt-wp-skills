...
Every flag `mkwp` itself accepts for site identity, ownership, and content is derived from context where the conversation already supplies it, and otherwise presented as a recommendation behind an accept-or-override gate — the same shape `clone` and `pull` use. `NAME` is the one value with no sensible universal default, so it is always settled first, from context or by asking directly. `--dirname` defaults to `NAME` (mkwp's own default) **even when the site is meant to mirror a domain** that is, or will become, a production site reachable by a later `/clone` — a `--dirname` diverging from `NAME` is known to break the scaffold outright against every `mkwp` version verified so far (see the caveat below), so that domain's full host (scheme, userinfo, port, and path stripped, `www.` and every dot kept — the same convention `clone` uses for its own directory naming, issue #11) is offered only as a declined-alternative behind an explicit failure warning, never as the default recommendation. `--yes` accepts every recommendation, never pauses, and prints the full decided-and-done record.
...
**Known upstream caveat.** `mkwp` 1.7.0 has a live-verified defect where a `--dirname` that differs from `NAME` breaks the scaffold outright (a database-connection error before `wp-config.php` is ever written), because `mkwp`'s own `ddev config` call lets DDEV register the project under the directory's name while `mkwp` still assumes the project is registered under `NAME`. This skill checks for the failure from the run log (never the on-screen output directly, see below) and diagnoses it precisely rather than surfacing a raw error dump, then cleans up the partial DDEV project and directory before reporting; the same risk applies to `clone`'s own `--dirname` usage (issue #11) whenever a production host's derived directory name differs from its derived project slug.
...
| `--dirname` | The directory the site is created in, underneath `--directory`. Recommended default: `NAME`, kept even when mirroring a future production domain — a diverging value is known to fail against `mkwp` ≤ 1.7.0 (see the caveat above); the full host is offered only as an explicitly-opted-into, warned-against alternative. |
...
Create it unattended, ahead of a future `/clone` for a known domain, opting into the mirroring `--dirname` explicitly despite the known `mkwp` ≤ 1.7.0 failure — the directory lands under the domain's full host so the later clone needs no rename, but confirm `mkwp` is patched past the defect first, or expect the scaffold to fail and need the cleanup described above:
