...
The skill reaches production solely through the Kntnt Extractor plugin's REST API, authenticated with an Application Password; there is no SSH. Before anything else it runs a health check that verifies every local and production dependency the run needs — `ddev`, `mkwp` ≥ 1.8.1, the required CLI tools, and the target site's Kntnt Extractor endpoint (its API-version handshake and that the Application Password's user holds both required capabilities) — with guided remediation the moment anything is missing, then proves the channel is live, confirms it targets production rather than the local site, sweeps stranded jobs from an aborted earlier run, and preflights the download path. It then discovers the site's shape, submits an extraction the Extractor plugin runs as a background job on production, downloads and unseals it, consumes the remote job immediately, and imports the result into DDEV. Localisation regenerates the affected thumbnails, rewrites URLs to the DDEV host, and flushes rewrite rules with plugins loaded so language routes survive.

Every decision is presented as a recommendation with an accept-or-override gate — which tables to carry with their data, which heavy blobs to leave behind, which `wp-config.php` defines to port. Form-submission tables (contact-form entries and the like) are excluded by default behind their own gate, since they are the most privacy-sensitive data the copy carries; accept carry there if you need real entries to debug a form locally. In interactive mode you walk the gates; under `--yes` every recommendation is accepted, nothing pauses, and a full record of what was decided and done is printed for you to read on return.
...
The finished copy is verified against a deterministic expectations file — core version, DDEV pins, table prefix, entity counts, table row-counts, drop-in absence, sample URLs derived from the copy itself, a database check, and more — by `skills/clone/scripts/smoke_test.py`, which also runs standalone from a terminal for a manual re-check or against a hand-edited baseline: `uv run "${CLAUDE_PLUGIN_ROOT}/skills/clone/scripts/smoke_test.py" <clone-directory> <expectations.json>`. It exits `0` when every activated check passed, `1` when the checks ran and at least one reported `fail` — the one outcome that says the copy is defective — and `2` when it could not run at all, so a caller never mistakes a bad invocation for a bad copy ([ADR-0028](https://github.com/Kntnt/kntnt-wp-skills/blob/main/docs/adr/0028-a-phase-fails-on-a-defective-copy-never-on-a-non-zero-exit.md)). Its `--generate` mode derives an expectations file from a discovery document instead of hand-writing one.
