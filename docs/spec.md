...
- **Version guard.** Before anything else, verify the local `mkwp` on `PATH` supports `--dirname` — the floor is `mkwp` ≥ 1.5.0 ([Kntnt/mkwp#2](https://github.com/Kntnt/mkwp/issues/2)) — and abort with precise install guidance if it is missing or too old. This is the same guard `clone`'s own health check (§ *Health check*, above) runs as part of its dependency step; every caller reads the single verdict-and-remediation helper `scripts/mkwp_guard.py` rather than deriving the check independently.
