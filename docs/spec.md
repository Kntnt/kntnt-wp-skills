...
Exclude from transfer exactly the DB-known generated sizes whose filename is what `wp media regenerate` would itself derive from the attachment's current attached file — `<stem>-<W>x<H><ext>` — because only a name regeneration can reproduce is safe to drop; pull whole everything not in that set, both because only DB-registered attachments can be regenerated at all and because a registered size whose name has drifted (a historical bulk WebP conversion, a dedup-suffixed PDF preview) can never be rebuilt under its production name. Regenerate after import — all attachments at clone, the metadata-driven delta at pull (compare each attachment's registered sizes against the files on disk, restricted the same way: an attachment joins the regeneration set only when a regenerable-named size is missing, never for a gap only a non-regenerable name could fill) — with `--regenerate-all` as the escape hatch. Full rationale in [ADR-0011](./adr/0011-metadata-driven-thumbnail-regeneration.md).
