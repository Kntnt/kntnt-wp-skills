...
- A fourth, standalone skill, `build-ollie-site`, builds a site out on the Ollie block theme (tokens → component patterns → section patterns → pages) and shares none of the transfer engine's machinery — not the health check, not the recommendation gates, not the Novamira control channel. Its design is specified separately in [`skills/build-ollie-site/DESIGN-RATIONALE.md`](../skills/build-ollie-site/DESIGN-RATIONALE.md); this document covers only its place in the plugin (§ *The `build-ollie-site` skill*, below).
...
- **Narrower requirements** than the engine's: an Ollie-parented install and WP-CLI; `uv` for the helpers; optionally the Ollie Abilities / MCP for content operations. It needs neither Novamira nor `mkwp`.
