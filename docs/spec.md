...
- **Its own helpers**, under `skills/build-ollie-site/scripts/` and *not* the transfer engine's helper seam: `dump_ground_truth.py` (resolved tokens and registered patterns from the live install), `mine_structures.py` (the pattern-cartography first pass), `lint_markup.py` (the on-system gate), and `check_contrast.py` (the WCAG-AA gate).
