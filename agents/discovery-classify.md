...
- `plugin_root` — `${CLAUDE_PLUGIN_ROOT}`, so you can locate `scripts/unseal.py`, `scripts/bootstrap_parse.py`, `scripts/discovery.py`, `skills/mkwp/scripts/classify.py`, and `scripts/poll_extraction.py`.
...
4. Pipe that document to `uv run "${plugin_root}/skills/mkwp/scripts/classify.py"`. Write its stdout to `<work_dir>/classifications.json`.
