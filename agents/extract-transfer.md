...
- Never return without a `DONE` or `FAILED` verdict, and never spend a poll loop one tool call at a time — wait inside a single bounded shell loop that terminates itself and prints its own verdict.
