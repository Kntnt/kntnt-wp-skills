...
`manifest.php` takes no exclusion payload and echoes an object of the shape `{ "entries": [ { "path", "size", "mtime" }, ... ], "unreadable": [ "path", ... ] }`: **every** file of production's content tree, anchored at the WordPress root, unfiltered (issue #18) — the exclusion set never travels to production as part of a manifest request. `unreadable` lists any subtree the walk could not descend into (a root-owned cache dir, a restricted upload subtree) — detected proactively rather than left to `CATCH_GET_CHILD` to swallow silently, since a silently-shrunk tree would misclassify that subtree's files as production-deleted. The runtime skill pipes this raw object, alongside the resolved exclusion set, to `uv run scripts/filter_manifest.py`, which restricts the entries to the in-scope subset and attaches the resolved set as `{ "scope": { "exclusions": [...] } }` — the shape `scripts/baseline_diff.py` has always consumed as the `current` side of its input alongside the stored last-sync baseline — or aborts loudly instead when `unreadable` is non-empty, since the deletion gate cannot trust an incomplete tree. After a successful run, that locally-filtered object is stored verbatim as the next run's baseline. `filter_manifest.py`'s `is_excluded` mirrors `baseline_diff.py`'s `is_excluded` exactly, so the manifest is filtered under the very scope the deletion diff later re-tests against. The worked examples of the diff's input and output are the `baseline-diff-*` fixtures under `tests/fixtures/`.
