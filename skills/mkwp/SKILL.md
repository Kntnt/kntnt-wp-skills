...
- **`--dirname`.** Default: `NAME` (mkwp's own default — omit the flag). If the operator names a domain this site is meant to mirror once it becomes, or already is, a production site reachable by a later `/clone`, recommend that domain's full host instead: pipe `{ "site": { "home_url": "https://<domain>" } }` to `uv run scripts/classify.py` and read `classifications.project_name.directory_name` — the exact function `clone` uses for its own directory naming (issue #11), so a later `/clone` into this same directory needs no rename. Always quote the resolved value in the final command (`--dirname="<value>"`) — the same defense-in-depth `clone` applies against an operator-corrected value carrying a shell metacharacter.
  ...
  **Known `mkwp` defect when `--dirname` diverges from `NAME` (observed against `mkwp` 1.7.0, live-verified 2026-07-19).** `mkwp`'s own `ddev config` call never passes `--project-name`, so DDEV registers the project under the *directory's* own name (its ordinary default when no `--project-name` is given) — yet `mkwp` still hardcodes the wp-config database host to `ddev-<NAME>-db`, assuming the project was registered under `NAME`. Whenever the two differ, `wp config create` fails outright with a database-connection error before `wp-config.php` ever exists, and every later step in the same `mkwp` run cascades from there. This is not something this skill can safely paper over from the outside — the failure happens *inside* `mkwp`'s own single synchronous run, before control returns here. After running `mkwp`, check that `wp-config.php` now exists in the resolved directory; if it does not and the captured output shows a `getaddrinfo for ddev-<NAME>-db failed` / database-connection-error shape, abort with a precise diagnosis naming this defect explicitly — do not blindly retry, and do not present the raw wp-cli error dump as if it were self-explanatory. **This same mismatch is a real risk to `clone`'s own already-shipped `--dirname` usage** (issue #11) whenever a production host's derived `directory_name` differs from its derived `project_name` — the common case for any `www.`-prefixed domain — so this is flagged here as a known upstream risk for a follow-up issue against both `Kntnt/mkwp` and this plugin, not silently worked around.
...
- **`--plugins`.** Default: **Novamira** — recommended every run, because the site needs it the moment it becomes a production site `clone`/`pull` reach ([ADR-0001](../../docs/adr/0001-novamira-mcp-sole-control-channel.md)); append any plugin context names after it, comma-separated. **Forward-pointer:** once the parked companion-plugin epic ([issue #24](https://github.com/Kntnt/kntnt-wp-skills/issues/24)) replaces the control channel, this recommendation switches to the companion plugin instead — do not keep recommending Novamira past that point.

  **Resolve the exact URL at run time — never hardcode a version.** Novamira has no WordPress.org listing; it is distributed from `https://github.com/use-novamira/novamira`. Two URL shapes both *look* plausible and are both wrong: the bare repo URL is a web page, not a zip, and fails `wp plugin install` outright; a `.git`-suffixed clone URL (the shape `mkwp`'s own git-repository support expects — live-verified to mechanically install) installs a plugin WordPress can see, but it is the raw source checkout, missing Novamira's bundled `vendor/` directory that its own release build assembles — it installs, then refuses to activate ("installed without its bundled vendor directory… install the Novamira release build ZIP instead", live-verified against v1.9.2). The value that actually works is the **latest GitHub release's packaged zip asset**: resolve it with `curl -sL https://api.github.com/repos/use-novamira/novamira/releases/latest`, read `.assets[0].browser_download_url` (a URL of the shape `https://github.com/use-novamira/novamira/releases/download/<tag>/novamira-<version>.zip`), and use that resolved URL as the `--plugins` entry — never a literal version pinned in this file, since it goes stale the moment Novamira cuts a new release.
...
Confirm the scaffold actually succeeded before reporting success: check that `wp-config.php` now exists in the resolved directory. If it does not, and the resolved `--dirname` differs from `NAME`, this is very likely the known `mkwp` defect documented at `--dirname` in §3 above — diagnose it as that explicitly rather than presenting the raw `wp-cli` error dump.
