...
7. **Verify the mkwp capability.** Run `mkwp --help` locally and confirm it lists `--dirname` — the floor for `mkwp` ≥ 1.5.0 ([Kntnt/mkwp#2](https://github.com/Kntnt/mkwp/issues/2)), the flag the scaffold step (§4) relies on. If it is missing or `mkwp` is not on `PATH` at all, abort with a precise report naming the required version — the operator installs binaries, this skill does not.
