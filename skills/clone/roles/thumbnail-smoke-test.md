...
3. If `smoke_test` is `true`: write `expectations` to `<scratchpad_dir>/smoke-test-expectations.json`, then run `uv run scripts/smoke_test.py <clone_dir> <scratchpad_dir>/smoke-test-expectations.json --log <scratchpad_dir>/smoke-test-report.json`. `--log` writes the full report to that path and prints only the compact summary — `ok`, the pass/fail/attention/skip counts, `sample_urls` (where the fetched URLs came from), the report's path, and every `fail` or `attention` finding — so the routine passes never cross into anyone's context. It runs every check itself — the URL fetches over sample URLs it derives from the copy itself, unless the orchestrator's `expectations` object overrode them with a `sampleUrls` list of its own (asserting a success response and the **absence** of `There has been a critical error`, `Fatal error`, and `Error establishing a database` in the HTML), `ddev wp db check`, entity and table-row counts, drop-in and object-cache-state checks, and the escaped-slash JSON asset-leak check — and the written report holds `ok`, a `summary` of pass/fail/attention/skip counts, and a `checks` list, each with `id`, `status`, and `detail`. Filter cosmetic WP-CLI/MariaDB deprecation notices from its stderr — they are never failures.
...
- `sample_urls_origin` — `derived` when the script asked the copy for its own sample URLs, `supplied` when the `expectations` object carried a `sampleUrls` list that replaced them, `none` when there were none to fetch; copied from the script's own `sample_urls.origin` summary field, never inferred. A supplied list is not an error, but it is the one expectation input a caller can plausibly build wrongly, so a later reader has to be able to tell it from a derived one (omitted when `smoke_test` is `false`)
