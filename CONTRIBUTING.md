...
4. **Run the tests.** The Python helpers — the transfer engine's under `skills/clone/scripts/`, the version guard and the classifier under `skills/mkwp/scripts/`, and the manual-page renderer under `scripts/` — are covered by a pytest suite under `tests/`. One command runs it, provisioning pytest through `uv`:
